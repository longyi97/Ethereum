...
public class GetEthTransactionService extends Thread {
	...
	private void insertRecord(Transaction transaction, String txInfo) {
		String sql = "insert into ruiec_etc_transaction_record (create_time,tx_hash,tx_status,from_address,to_address,value,tx_info,type) values(?,?,?,?,?,?,?,0)";
		JSONObject jsonObject = JSONObject.fromObject(txInfo);
		List<Object> list = new ArrayList<Object>();
		list.add(new Date());
		list.add(transaction.getHash());
		list.add(jsonObject.optBoolean("TxReceiptStatus"));
		list.add(transaction.getFrom());
		list.add(transaction.getTo());
		list.add(jsonObject.opt("value"));
		list.add(txInfo);
