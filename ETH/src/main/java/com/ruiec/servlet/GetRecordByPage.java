...
public class GetRecordByPage extends HttpServlet {
		...
		try {
			...
			if (result.size() > 0) {
				for (Map<String, Object> obj : result) {
					JSONObject jsonObject = JSONObject.fromObject(obj.get("transaction_json"));
					jsonObject.put("dbid", Integer.valueOf(obj.get("id").toString()));
					transactionRecord.add(jsonObject);
				}
				LOGGER.info("查询交易记录成功", transactionRecord);
				response.getWriter().println(ResponseJSONObject.ResponseJSONList(200, true, "查询交易记录成功", transactionRecord));
			} else {
	...
	private List<Map<String, Object>> findRecordByPage(Integer id, Integer pageSize) {
		...
		try {
			...
			String sql = "SELECT id,transaction_json from ruiec_eth_transaction_record WHERE id > ? LIMIT 0,?";
