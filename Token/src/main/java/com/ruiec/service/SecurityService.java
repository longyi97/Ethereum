...
public class SecurityService {
	 ...
	 */
	public Boolean dailyAmountLimit(BigDecimal amount, String token) {
		try {
			BigDecimal todayAmount = findTodayAmount(token);
	 ...
	 */
	public BigDecimal findTodayAmount(String token) {
		String sql = "SELECT SUM(`value`) amount FROM ruiec_token_transfer_request WHERE contract_address = ? AND TO_DAYS(create_time) = TO_DAYS(NOW())";
